...
func Dial(options RTSPClientOptions, quit *int) (*RTSPClient, error) {
	client := &RTSPClient{
		...
		if i2.AVType == VIDEO {
			if i2.Type == av.H264 {
				if len(i2.SpropParameterSets) > 1 {
					...
					//终端2.0版本RTSP推流编码处理
					if strings.Contains(client.options.URL, "session0") {
						client.Println("Terminal 2.0 rtsp: ", client.options.URL)
						client.WaitCodec = false
					}

					if strings.Contains(client.options.URL, "Channels") {
						client.Println("Channels rtsp: ", client.options.URL)
						client.WaitCodec = false
					}

					if strings.Contains(client.options.URL, "tracks") {
						client.Println("Tracks rtsp: ", client.options.URL)
						client.WaitCodec = false
					}
	...
	// client.Println("rtsp client spends: ", time.Now().UnixNano()/1e6-startTime)

	tempFlag := 0
	client.flag = &tempFlag

	go client.startStream(quit)
	return client, nil
}

func RTSPHeartbeatDetect(options RTSPClientOptions) (*RTSPClient, error) {
	client := &RTSPClient{
		headers:             make(map[string]string),
		Signals:             make(chan int, 100),
		OutgoingProxyQueue:  make(chan *[]byte, 3000),
...
func (client *RTSPClient) startStream(quit *int) {
	defer func() {
		*quit = 0
	...
	for {
		...
		if *quit == 1 {
			client.Println("=============> rtsp stream quit......")
			*quit = 0
			break
		}
		...
		case 0x24:
			...
			for _, i2 := range pkt {
				...
				// client.Println("RTSP send packet: ", client.options.URL, len(i2.Data), i2.IsKeyFrame)
